...
`HubIPC/SecretLeakGuard.swift`，跑在 **hubctl 本地**。

### 为什么不在 app 里

仓库其它拦截都是 app 侧决策，app 没开就放行（「一个监控工具不该让你的命令跑不起来」）。
这一条反过来：**密钥漏出去是不可逆的**，一次就够。所以它不依赖 app 在不在跑。

代价是它拿不到加密库里的值，只能读**已经物化到磁盘**的那几个 `.env`。
这是刻意的 —— 那些文件本来就是明文、就是给 AI 用的，hubctl 读它们不增加暴露面。
关掉「物化到磁盘」之后值级拦截跟着失效，但那时候磁盘上本来也没有值可漏。

### 判什么

1. **参数里出现真实密钥值** → 拒。`flattenStrings` 递归摊平整个 `tool_input`，
   嵌套的（`MultiEdit` 的 `edits` 数组）也看 —— 只看顶层的话，
   把值放进数组里就绕过去了。
2. **`git add` / `git commit` 会把密钥带进仓库** → 拒。分两档：
   文件名像密钥文件（`.env` / `id_rsa` / `*.pem` / `credentials.json` …），
   或者待提交的普通文件内容里有真实值。

### 几条必须守住的边界

- **值短于 8 个字符不参与匹配。** `abc` / `true` / `test` 会命中一切，
  一旦有人往库里存了个短值，全项目的正常编辑都会被拦。
- **`.example` / `.template` / `.sample` / `.dist` 结尾的放行。** 那是给人看的骨架，
  提交它正是正确做法。误拦一次正常提交，用户就会把整个功能关掉，
  那连该拦的也拦不住了。
- **只认 `EnvRenderer` 写出来的那个格式**（`export KEY='值'`），不做通用 dotenv 解析。
  解析器判宽了会把注释里的片段当成值，然后到处误伤。
- **不是 `git add` / `git commit` 就一个 git 子进程都不起** ——
  这道闸挂在每一次工具调用前面。
- **fail-open。** 起不了 git、不在仓库里、输出看不懂，一律放行。

### 拒绝理由要给出正确做法

只说「被拦了」的话，模型会换个写法再试一次（换成 `printf` 写、换成 base64、
分两次写一半）。所以理由里明确写「用 `$KEY`」「先加 .gitignore」。

### 可见性和开关

hubctl 拒了之后仍然把事件发给 app（不等应答），
`HookCoordinator` 把它记进审批日志 —— **一道看不见的闸，
用户第一次被拦时只会觉得 Claude 抽风**。

开关在密钥页底部，落成 `~/.vibe-foreman/guard.json`。
**只在关掉时才写这个文件**：默认开，而「文件不存在 = 开着」是 hubctl 那边的约定，
这样一个从没打开过密钥页的用户也受保护，也不会凭空多出一个 `~/.vibe-foreman/`。

### 它挡不住什么

`python -c` 读文件、软链绕路、base64 转一道、密钥库之外用户手写的值 —— 都拦不住。
**它是一道减少手滑的闸，不是沙箱。** 文案和注释都按这个写，不吹成安全边界。
