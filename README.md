...
这个仓库里是**应用和文档**，不含源代码，所以没套源码许可证 ——
那会承诺一些这里给不了的东西。分两块，详见 [LICENSE](LICENSE)：

- **应用本体**：免费，个人和商业都可以随便用、公司内部部署也行。
  可以原样转发安装包（请指向本仓库，好让别人拿到更新和校验和）。
  不提供源码，按「原样」提供、不含担保。
- **文档与截图**：[CC BY 4.0](https://creativecommons.org/licenses/by/4.0/deed.zh)，
  注明出处即可自由引用转载。
