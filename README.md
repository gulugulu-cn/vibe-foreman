...
还有第四件，它比前三件贵得多：**你不知道它把你的密钥搬到哪儿去了**。
`git add .` 顺手把 `.env` 提了、把真实 token 内联进配置文件当"示例"、
为了"确认一下配置对不对"把密钥 `cat` 出来 —— 每一步单看都很正常，
而结果是不可逆的。Hub 默认拦住这一类操作，[往下翻](#ai-在替你干活也在替你泄密)。
...
这一节是这个工具最近才长出来的部分，但它可能是最值钱的一部分。

把活交给 agent 之后，泄密的形态变了。以前是人手滑，现在是**每一步都很合理的一串操作**：

| 它做了什么 | 看起来 | 实际结果 |
|---|---|---|
| `git add .` 然后提交 | 正常收尾 | `.env` 进了仓库，推上去就再也撤不干净 |
| 把 token 内联进配置文件 | 在写示例 | 真实密钥进了代码，还会被下一个人复制走 |
| `cat .env` 确认配置 | 在排查 | 值进了 `~/.claude/projects/*.jsonl`，0644、永久、每轮重发给 API |
| 在回复里贴一段"完整调用示例" | 在解释 | 密钥进了对话记录，也进了你截图发给同事的那张图 |

**没有一条能靠"这条命令危不危险"判断出来。** 它们都不危险。
唯一可靠的信号是：**这次调用的内容里，有没有出现真实的密钥值。**

所以 Hub 装的那套 hook 里有一道闸，**默认开着**，在工具真正执行前拦：

- 参数里出现了密钥库里的真实值（写文件、内联进命令、塞进 `MultiEdit` 的数组里都算）→ **拒绝**
- `git add` / `git commit` 会把 `.env`、`id_rsa`、`*.pem`、`credentials.json` 这类文件带进仓库 → **拒绝**
- 待提交的普通文件里藏着真实密钥值 → **拒绝**

拒绝理由里会告诉它正确做法（用 `$KEY`、先加 `.gitignore`），不然它只会换个写法再试一次。
`.env.example` 这种模板文件照常放行 —— 误拦一次正常提交，用户就会把整个功能关掉，
那连该拦的也拦不住了。

**这道闸和其它拦截反过来：它不依赖 app 在不在跑。**
仓库里别的审批都是"Hub 没开就放行"（一个监控工具不该让你的命令跑不起来），
但密钥漏出去是不可逆的，一次就够 —— 所以它跑在 `hubctl` 本地，
你有没有开着岛都一样。被拦的每一次都会进审批日志。

它挡不住什么，也一并写在这里：`python -c` 读文件、软链绕路、base64 转一道，
以及密钥库之外你自己手写的那些值 —— 这些都拦不住。
**它是一道减少手滑的闸，不是沙箱。**

### 平台密钥配一次，给 AI 一个路径

<img src="docs/screenshots/10-secrets.png" width="720" alt="共用密钥：按平台建组，勾给项目">
...
**那道密钥闸会不会拦错东西？**
它只认**你放进「共用密钥」里的那些值**，以及 `.env` / `id_rsa` / `*.pem` /
`credentials.json` 这类文件名（`.example` / `.template` / `.sample` 结尾的放行）。
普通代码、普通提交一概不碰。真被误拦了，密钥页底部有开关能关掉 ——
但先看一眼审批日志里的拒绝理由，多数时候它拦对了。
...
以及 `~/.vibe-foreman/`（共用密钥物化出来的 `.env`，`0700` 目录 + `0600` 文件，
**只有你在密钥页勾了项目之后才会出现**；另外你把密钥闸关掉时会多一个
`guard.json` 记这个选择）。
**不往你的项目目录里写任何东西。**
...
hook 链路、跳转算法，跑在你机器上的每一行都在这个仓库里。零第三方依赖，646 个测试。
