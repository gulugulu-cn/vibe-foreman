...
用户选了这一侧（默认开着，设置里能关），但紧接着补了一句
**「那我也打不开了呀」**。只会杀不会还的回收器，是把一个显示错误的会话
换成一个找不回来的会话 —— 那不是修好，是换了个方式弄丢东西。

所以 `SessionReaper` 杀之前先把 sessionId 记进 `ClosedSessionStore`，
项目行和岛的启动菜单上长出「接着上次」，走 `claude --resume <id>`。

**但 `claude --resume` 认的是磁盘上那份 transcript，不是这个 id。**
一次都没说过话的会话压根没有 transcript 文件 —— 实机验证过：
一个 11:35 起来、只停在提示符上的会话，`~/.claude/projects/…` 里
一个字节都没有。不查就记的话，界面上会长出一个点下去 claude 报错退出的
按钮，比没有这个按钮更糟。判据用 `TranscriptReader.locate`。

回收器另外三条边界：
- **绑不到 pane 的一个都不许碰。** 不在 tmux 里的会话（VS Code 扩展、
  直接开的终端、bg 任务）判断不了"有没有终端连着"，杀它们等于拿"不知道"
  当处决理由 —— 这一档最容易写漏，漏了就是批量误杀。
- **新起的会话有 90 秒宽限。** `createSession` 是先建 detached 的 tmux
  会话、再让终端去 attach 的，iTerm 冷启动那几秒客户端数就是 0。
  没宽限期的话回收器会把用户刚点开的项目当场杀掉，现象是"点了没反应"。
